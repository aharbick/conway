...
### Tried and reverted: tiering the filter by lifetime

One filter answers a blunter question than the search asks. A pattern covered at generation
9 hunting for 215 needs a state living 206 more generations, but a miss in the >=180 filter
only proves "under 180" - and a state living 190 generations is a *hit* there, so the
pattern keeps simulating to 215 even though 9 + 190 could never have reached it. Building
several filters, one per threshold, and probing the most selective one that still answers
(the >=204 filter is 368 keys and 4KB) fixes that and makes the common probes cache-sized.

It was built, measured, and reverted. The gain was 1-3%, at the edge of what is
distinguishable on a busy GPU, and after reverting the same interval ran at 169/171/167 G/s
against the tiered 171/168/168.

The number that explains it, and that should have been computed first:

```
tier1 discards 79.9%   tier2 discards 18.2%   kept 1.9%
never coverable: 1.8%
```

Of the 1.9% that survive the oracle, 1.8% never fit a 7x7 box at all, so tiering cannot
touch them. That leaves ~0.1% of patterns - the ones that get a filter hit and keep running
- as the entire population a more selective filter can improve. Everything else was already
discarded on a miss, which one filter does just as well.

A related idea fails for the opposite reason: replacing the >=180 filter with a single
higher-threshold one. A >=196 filter is 0.3MB and cache-resident, but it cannot decide
patterns covered at generations 19-34 at all, so they stop being pruned and run long -
measured 164/162/164 G/s, slower than the 33.6MB >=180 filter it replaced. The breadth of
the wide filter is load-bearing; its size is not the problem.

Worth ~400 lines in the one part of the search where a wrong answer is silent and
permanent? No. If the cache is ever rebuilt at a lower threshold the arithmetic changes and
this is worth revisiting, but not before.
